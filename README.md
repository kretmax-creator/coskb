...
Скрипт `scripts/backup_cron.sh` создаёт дамп через `backup_db.sh` и выполняет ротацию: хранятся только последние **N** дампов (`coskb_*.sql` в `data/backups/`), остальные удаляются. По умолчанию **N = 4**. Это удобно для еженедельного cron: за месяц накапливается до четырёх бэкапов.

- **Ручной запуск (проверка):** из корня проекта выполнить  
  `bash scripts/backup_cron.sh`
- **Настройка cron (например, каждое воскресенье в 3:00):**
  ```bash
  crontab -e
  ```
  Добавить строку (путь заменить на свой каталог проекта):
  ```text
  0 3 * * 0 cd /home/user/coskb && bash scripts/backup_cron.sh
  ```
- **Количество хранимых дампов:** в `.env` можно задать `BACKUP_KEEP_COUNT=4` (по умолчанию 4). При превышении лимита старые дампы в `data/backups/` удаляются автоматически.
