...
- [ ] Определить внутренний и внешний IP VM (`ip addr`, `curl ifconfig.me`)
- [ ] Проверить доступность порта 8890 извне корпоративной сети
- [ ] Проверить доступ VM к `api.telegram.org` (`curl -s https://api.telegram.org`)
- [ ] Задокументировать результаты в `docs/network-research.md`
- [ ] Принять решение по доступу извне (по результатам исследования)
